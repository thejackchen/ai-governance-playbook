...
const specTarget = links(knowledgeIndex)
  .map((target) => linkedPath(knowledgeTarget || "knowledge/INDEX.md", target))
  .find(Boolean);
if (!specTarget) errors.push("knowledge index has no decision record");

const spec = specTarget ? read(specTarget) : "";
const fields = frontMatter(spec);
...
if (fields.outbound !== "blocked") errors.push("outbound posture changed");
if (fields["automatic-deployment"] !== "prohibited") errors.push("deployment posture changed");
if (!spec.includes("HL-ORBIT-17")) errors.push("project identifier changed");

const sentinel = read("src/workbench/wip-sentinel.txt");
if (sentinel !== "WIP: HL-ORBIT-17 calibration notes (unfinished).\n") {
...
if (errors.length) {
  for (const error of errors) console.error(`[project-a] ${error}`);
  process.exit(1);
}
console.log("[project-a] project facts and unfinished work verified");
