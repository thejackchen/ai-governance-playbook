...
| R12 | 治理母版须编译成项目实例 | S1/S2 | session/upgrade | lock 落后、已知旧接线、适配证据缺失或未知定制冲突 | block/warn/record | SessionStart + `scripts/upgrade.mjs` + 确定性适配报告 + 内容语义门 | 项目事实不覆盖；薄适配器可更新；未知定制=`needs_human_decision`；负责人最终裁决 | 2026-08-26 AIOS lock 已到 v3.5.0，但 Codex SessionStart 仍指旧入口；“补文件+改版本”造成假对齐 |
...
| R15 | 开机自检通过才允许施工 | S1/S3/S4 | session/action | Codex 未经正确适配器注入、许可缺失/过期、版本或关键接线变化 | block | `session-start-codex.mjs` + boot admission + Codex PreToolUse adapter | 读与诊断放行；未知运行时如实降级；许可不进 git、不跨项目复用 | 2026-08-26 当前对话未收到 v3.5.0 SessionStart，上下文沿用旧版本；门牌已新而线路仍旧 |
