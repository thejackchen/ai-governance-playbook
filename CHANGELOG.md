...
## 2026-09-07 · v4.3.0 · 语义升级正式发布
- [release] 实现 `431a6cda0fed2c288d528302ed83574757580204`；PR #6 已正常合并；实施发布基线 `6cf12a59d8826e37132e2a6f31e31671cf061ec2`。GitHub API 与 git fetch 回读 `VERSION=4.3.0`，deterministic CI run `34107998818` 成功。
- [verify] 主代理全量 162/162、专项 27/27；无上下文初始 A 82 分失败，修订模板后全新 A 96 分、冲突 B 96 分。doctor 0 error/1 warning；声明源码 150/150、文档 23/23、地图 31 行/3250 bytes。评分、反例和范围见语义升级验收正本。
- [boundary] 模板给语义建议，项目 AI 自主适配并验证后前移采用版本，无逐仓审批清单；脚本仅发现与提示。全消费仓采用、真实客户端 UI trust 未验；远端 advisory 模型步骤被跳过，独立审查由主代理及本轮无历史实作承担。母版仅配置 GitHub，无 Codeup，不改业务/部署/主机。

## 2026-09-07 · v4.3.0 候选 · 语义适配升级纠偏
- [decision] 负责人澄清：模板给出语义建议，项目发现已发布新版后由本项目 AI 自主理解、适配、验证，无需负责人逐仓或逐能力指定；仅无法解决的真实语义冲突交负责人。v4.2.0 将“禁止脚本盲覆盖”扩大为“禁止自主适配”的限制被取代。
- [scope] 版本发现与项目适配分离：Hook 只给活跃 AI 任务，不调用模型或写治理文件；直接 discovery 文件安装仍是可选工具，不能以文件安装/只改版本号冒充采用。新版候选验收见 REQ-GOV-004 与语义升级前向测试，尚未发布。

## 2026-09-07 · v4.2.0 · 项目发现能力母版正式发布
- [release] 实现提交 `42a1a3f55645caa29656ea88e195d726925ee79c`；PR #4 已合并；GitHub 默认分支 `985a03710d128281cfcfb5333237e636bc837b78`；GitHub API 与 `git fetch` 回读 `VERSION=4.2.0`，母版主目录 `main` 已安全 fast-forward 同 SHA 且 clean。
- [verify] 母版 CI `140/140`；doctor `0 error/1 warning`（Codex 新会话 trust）；文档覆盖 `22/22`、已识别源码覆盖 `127/127`，独立冷启动导航通过。真实客户端 UI trust 与全客户端现场未验，不作宣称。
- [scope] 仅母版 v4.2.0 发布完成；跨仓采用仍待逐项确定消费者采用分支、owner 与验收，本条不进入业务部署分支。

## 2026-09-07 · v4.2.0 候选 · 项目发现能力母版
- [feat] 增加通用元数据索引与八域地图，按稳定资产 ID、别名、标签和一跳关联导航项目事实。
- [safety] 发现能力按能力显式安装；停止 SessionStart 或旧启动入口中的自动 kit 写入升级，不恢复已退出的自动迁移。
- [evidence] 在已识别范围内，阶段检查记录源码覆盖 `127/127`、文档覆盖 `22/22`；这只是范围内证据，不推断未识别范围。
- [verify] 母版完整测试已有阶段通过记录；无历史新会话验收、远端发布仍待最终主验。
- [status] 本条为发布候选，尚未正式发布，也不能声称所有消费端已采用；各消费项目仍须逐项记录实际采用、验证与同步状态。

## 2026-08-31 · v4.0.1 · governance-optimization skill 收编正本
- [feat] `skill/governance-optimization/` 收编为正本（v1.1）：治理架构减重与验效编排器（量化盘点→第一性判决→一次拍板→分批拆除→冷启动考试+减重不减防双测→沉淀回写）。`~/.claude/skills`、`~/.codex/skills` 降为安装实例，换环境从本仓复制。
- [docs] skill v1.1 增补四条判例：上游收编要过对方合同测试（8 红回退实证）、发版仪式多处正本+指纹缺一必拦（好门标本）、共享暂存区是并发资源（按路径提交）、工具指纹失明盘点法（Cursor 判例）。

## 2026-08-31 · v4.0.0 · 第一性原理拆除教义（破坏性）
- [docs] 判例新增《治理机器按第一性原理拆除》：每件零件答「防的真实失败是什么（要事故出处）？有无更便宜替代？」答不出即拆。已判死刑模式：worktree 生命周期登记制（git 原生盘点+人工确认删除取代）、语义门进热路径（降按需命令，体量门接管背压）、kit 自动升级（开工 hook 不再拉外部仓写入本仓）、拉模式面板、治理自我保护栓。保留判据：事故出处/廉价正则/负责人感知铭牌；为凑指标削活火警=为好看牺牲求真。
- [breaking] 分支正本模板九段→四段：当前游标/现役规格/事实与边界/待负责人裁决；使命+阶梯进前言区；风险与验收记录降级为判决记录（【边界】行免证据指针）。教义：语义文字是档案不是执法者。
- [docs] claim-gate 模板 agent 值域补 `grok`/`cursor` 与环境指纹说明（AIOS 判例：Cursor 会话此前记 unknown 且不触发认领门，主战场失明）。
- [source] AIOS Demo5 一夜四批拆除实证：净删约 1.5k 行治理代码，doctor 15 WARN→2 WARN，收工 hook 不再因 worktree 拦会话，开工 hook 12s→1s，89 测试绿；冷启动考试基线 99 分（减重不减防）。
- [todo→v4.1] 代码收编两件（首次尝试因 kit 合同测试锚定模板行为而回退，需配套改测试再收）：①模板 claim.mjs 收编 AIOS 演化版（自动收割 reap+Cursor 指纹+公告板 agent 列）；②模板 SessionStart 移除自动 pull+upgrade --write，降级为只读版本提醒。

## 2026-08-29 · v3.7.7 · 上游仓禁推机器护栏
- [docs] 判例新增《上游仓禁推要用机器护栏》：远端指向第三方/官方上游的仓（vendored fork、补丁仓），「提交即推远端」语义反转为泄漏；首次动仓先 `git remote -v` 判定推送语义，判定禁推当场 `git remote set-url --push origin DISABLED-...` 物理禁用，远端语义与发布通道落项目正本。
- [source] AIOS Demo5：主 agent 在 WoC 工作区提交钉扎后惯性外推官方上游，被 403 拦截未泄漏；负责人当场纠正「语义漂移」。AIOS 侧已禁用该仓 push URL 并钉进 runbook 坐标表。
- [feat] 本版提交同时收编了此前遗留未提交的 SessionStart 输出压缩：仓外正本报告与认领公告板默认紧凑（只列未装载与同线并发计数），`--verbose` 看全量明细；含模板同步与回归测试，全套 97/97 通过后收编。

## 2026-08-29 · v3.7.6 · 仓外事实用过即登记
- [docs] 判例新增《运维通道用过即登记》；模板仓外正本指引「以后每个 AI 必须遵守」补第 6 条：实际作业用过的通道/凭据位首次使用当场登记专项条目，存在性条目不等于通道登记。
- [source] AIOS Demo5 部署备料：主 agent 找不到多环境反复使用的 Mini SSH 通道，只能向负责人求助；负责人裁决按登记缺口处理并要求边做项目边补治理。首例落地为 AIOS `mini-ssh-channel` 条目。
