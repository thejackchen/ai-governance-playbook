...
if (needsAdmission) {
  const admission = validateBootAdmission(root, { runtime: "universal" });
  if (!admission.ok) {
    process.stdout.write(JSON.stringify({
      decision: process.env.GROK_HOOK_EVENT ? "deny" : "block",
...
function isDiagnosticShell(rawCommand) {
  const commandText = String(rawCommand || "").trim();
  if (!commandText) return true;
  if (/(?:^|[^<])(?:>>?|<<?)|\b(?:tee|touch|mkdir|rm|cp|mv|install|chmod|chown|ln|truncate|dd|patch|apply_patch)\b/i.test(commandText)) {
    return false;
  }
