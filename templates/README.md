...
> 背后的「为什么」见上层 [`../governance-architecture-essence.md`](../governance-architecture-essence.md)；这里只讲「怎么用」。

## 怎么用（6 步）

1. **复制**：把 `templates/` 下所有文件复制到你项目根——`CONSTITUTION.md`、`AGENTS.md`、`CLAUDE.md`、`ROADMAP.md`、`CHANGELOG.md`、`docs/`、`KNOWLEDGE_BASE/`、`scripts/`、`.githooks/`、`package.json`。
...
4. **改 lint 配置**：`scripts/knowledge-lint.ts` 顶部 CONFIG 常量改成你项目的路径 / 阈值。
5. **首跑验证**：`npx tsx scripts/knowledge-lint.ts`，修到 0 error。
6. **验收**：治理大改后，按上层 [`../acceptance-test.md`](../acceptance-test.md)（复制并填好放到 `docs/runbooks/governance-acceptance-test.md`）派无记忆 AI 跑一遍。
...
docs/runbooks/
  ...
  ├── governance-acceptance-test.md      验收套件（指针 → 上层 acceptance-test.md）
...
不该建的（母版砍掉的官僚，别加回来）：控制点注册表 / 季审 / 仪式预算 / ROI 举证制度 / L1-L4 黑话 / 为凑行数硬删——见 [`../setup.md`](../setup.md) 的「⚠️ 别建这些」段。
