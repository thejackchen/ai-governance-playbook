# ai-governance-playbook · AI执行宪法

> 运行时：codex；治理Profile：standard。治理安装记录见`governance.lock.json`。

## 意图

本项目把负责人意图和软件红线编译为可触发、可判定、可留证的治理控制回路，并为Codex、Claude Code和通用运行时提供同一核心下的适配器。当前阶段与候选状态以ROADMAP.md为准：优先保证方法论不分叉、确定性门禁真实生效、存量迁移可回滚，以及无上下文AI能够正确安装和遵守治理。

本项目自身也是Standard Profile的参考实现。每条规则都应能回答触发、判定、效果和证据；无法自动执行的内容必须如实标记为人工或AI建议层。
...
- 需求权威：[需求](docs/requirements/backlog.md)。
...
- 只维护一份治理核心；适配器和Skill不得复制或发明第二套方法论。
- 确定性红线不得只依赖AI判断，必须优先落到脚本、测试、权限或CI。
- 自动化测试和无上下文前向测试未通过，不得发布新版本。
...
- 修改后运行：`npm run check && npm test`；治理载体改动另运行`node scripts/governance-verify.mjs --ci`。
