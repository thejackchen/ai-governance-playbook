...
- 执行力分层：本地=便利层（API 失败红字标注放行），远程 CI=权威层（失败禁止合并）；`needs_human_decision` 保持阻断并登记待裁决；紧急绕过必须留痕补审。
...
- 本仓记录：`governance.lock.json` 的 `playbookVersion`、kit 指纹和项目适配结果。lock 只在适配与最低验收完成后前移，不能把“下载过模板”冒充“项目已采用”。
...
- **开机自检**：Session Start 不只比版本，还验证当前运行时确实收到适配后的规则。通过才签发短期施工许可；写入入口验证许可，缺失、过期或关键载体变化时 fail-closed，读与诊断继续开放。
