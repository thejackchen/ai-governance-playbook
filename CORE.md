...
消费项目可以在 SessionStart 只读核对线上版本，不在每一轮对话、也不在每个
PreToolUse 查询。开机核对不拉取、不写入项目，也不会因为版本差异自动迁移；
施工前必须由负责人显式选择升级能力。离线时默认只做本地诊断，不猜测线上状态。
...
- 显式升级入口按能力执行，默认只输出只读计划；当前母版只提供
  `--capability discovery`，只有同时带 `--write` 才能安装发现能力文件。未选择能力的
  旧式写入请求必须拒绝，`full` 能力暂不提供。能力回执与基础母版版本、运行时 Hook、
  admission、policy 和项目事实分开记录。
- **治理编译**：母版先按消费项目的真实目录、权威文本、运行时和风险边界生成项目实例。项目自己的宪法、游标、业务文档和 policy 不由模板原样覆盖；负责人显式升级时，playbook 管理的薄适配器才允许随版本更新；能够精确识别的旧接线可做窄迁移；未知定制停止并交负责人裁决。
...
- **显式运行时升级分两次开机**：负责人显式修改运行时适配器后，lock 先记 `restart_required`，本会话禁止施工；下一次新会话使用新适配器并通过项目验证后才写 `pass`。未知定制或验证失败分别写 `needs_human_decision` / `failed`，避免旧进程继续用旧门发证。
- 本机 kit 领先 GitHub = 未发布，不把未发布版本写进消费仓 lock；候选版本在发布前不得写入完成状态。
- 离线且已有未过期的已验适配可降级继续，但只能按现有 lock 施工，不能借离线状态静默升级；适配冲突、接线失效或从未验收不得以旧 lock 冒充可施工。
