...
v4.2.0 母版发布部分已完成；本轮 v4.3.0 也已完成母版验收与发布回读，准确版本、实施发布基线、远端回读与母版验收证据见 [ROADMAP.md](../../../ROADMAP.md)。本节仅保留 v4.2.0 冷启动修复与消费盘点，真实客户端 UI trust 与全客户端现场仍不作宣称。
...
本次母版 v4.3.0 语义升级发布判据已满足：确定性测试、独立无上下文语义评分与固定来源/SHA 发布回读
均通过，证据见 ROADMAP 与 [语义验收](../../evals/semantic-upgrade-forward-tests.md)。本 REQ 按负责人纠偏后的
母版发布及自主适配机制范围完成，不以版本号或文件复制代替项目采用，不宣称所有消费仓采用；各消费项目
按自身事实记录采用、接线和验收，不再将历史盘点变成需要负责人逐仓拍板的升级清单。
