...
function validateSpecRefs(entry, root, backlogPath) {
  ...
  if (!refs.length) {
    errors.push(`[需求系统] ${entry.id} 的 spec_refs 不能为空`);
    return errors;
  }
  for (const ref of refs) {
    const specPath = resolve(dirname(backlogPath), ref);
    if (!isInsideRepo(root, specPath)) {
      errors.push(`[需求系统] ${entry.id} 的 spec_refs 必须位于项目内并相对 backlog 解析: ${ref}`);
      continue;
    }
    if (!existsSync(specPath)) {
      errors.push(`[需求系统] ${entry.id} 的 spec 文件不存在: ${ref}`);
      continue;
    }
    const spec = readFileSync(specPath, "utf8").trim();
    if (!spec || /TODO(?:\(|\[|:|\b)|<[^>]+>/.test(spec)) {
      errors.push(`[需求系统] ${entry.id} 的 spec 文件为空或仍含占位内容: ${ref}`);
    }
  }
  return errors;
}
...
function validateEntry(entry, root, backlogPath) {
  const errors = [];
  const { done, fields, id } = entry;
  const requiredFields = ["source_refs", "spec_refs", "acceptance", "evidence"];
  const missing = requiredFields.filter((key) => !fields[key]);
  if (missing.length) {
    errors.push(`[需求系统] ${id} ${done ? "已交付项" : "未完成项"}必须补齐字段: ${missing.join(", ")}`);
  }
  if (fields.evidence?.toLowerCase() === PENDING_EVIDENCE && done) {
    errors.push(`[需求系统] ${id} 已交付项不允许 evidence 为 pending`);
  }
  if (fields.spec_refs) errors.push(...validateSpecRefs(entry, root, backlogPath));
  return errors;
}
...
function main() {
  ...
  if (errors.length === 0) console.log("[requirements-check] PASS");
  else console.error("[requirements-check] FAIL");
