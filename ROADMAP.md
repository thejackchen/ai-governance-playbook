...
当前发布候选为 v3.7.1：母版 doctor 与项目验证器分层，项目验证失败不得签发许可；受管 Hook 必须唯一，许可指纹覆盖真实启动语义载体，热升级先阻断一次会话再由新适配器复验。v3.7.1 补正重启后复验的人话输出，待第二轮零上下文验收后结账。
...
| 治理编译与开机自检 | 推进 | v3.7.1 两层验证、Hook 唯一性、扩大指纹、热升级重启门与复验输出已实现 | 第二轮零上下文验收 | Codex 项目 Hook 仍需真实 UI trust | `docs/requirements/specs/REQ-GOV-003-governance-compilation-and-boot-self-test.md`、`docs/playbook-update.md` |
...
| v3发布 | 推进 | v3.7.1 发布候选施工中 | 完整测试、远端发布、AIOS 采用、零上下文复验；不创建正式tag | Codex 非托管 Hook 仍需真实 UI trusted + `/hooks` 审核验收 | `CHANGELOG.md` |
